...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        ...
        for param in p.parameters():
            ...
            verify_node(param, 'parameter', (1, dimensions),
                        'PerceptronModel.parameters()')

        assert number_of_parameters == 1, (
            'Perceptron Model should only have 1 parameter')
    ...
    for dimensions in range(1, 10):
        ...
        for param in p.parameters():
            expected_score = float(
                np.dot(point.flatten(), param.detach().numpy().flatten()))
    ...
    for dimensions in range(1, 10):
        ...
        for point in (random_point, np.zeros_like(random_point)):
            ...
            expected_prediction = np.where(
                np.dot(point, p.get_weights().data.T) >= 0, 1, -1).item()
    ...
    for multiplier in (-5, -2, 2, 5):
        p = models.PerceptronModel(dimensions)
        orig_weights = p.get_weights().data.reshape(
            (1, dimensions)).detach().numpy().copy()
    ...
    accuracy = np.mean(np.where(
        np.dot(dataset.x, model.get_weights().data.T) >= 0.0, 1.0, -1.0) == dataset.y)
    if accuracy < 1.0:
...
def check_regression(tracker):
    ...
    for batch_size in (1, 2, 4):
        inp_x = torch.tensor(
            dataset.x[:batch_size], dtype=torch.float, requires_grad=True)
        inp_y = torch.tensor(
            dataset.y[:batch_size], dtype=torch.float, requires_grad=True)
        ...
        grad_y = torch.autograd.grad(
            loss, inp_x, allow_unused=True, retain_graph=True)
        grad_x = torch.autograd.grad(
            loss, inp_y, allow_unused=True, retain_graph=True)
    ...
    verify_node(train_predicted, 'tensor',
                (dataset.x.shape[0], 1), "RegressionModel()")
...
def check_digit_classification(tracker):
    ...
    if test_accuracy >= accuracy_threshold:
        ...
        print("Your final test set accuracy ({:%}) must be at least {:.0%} to receive full points for this question".format(
...
def check_lang_id(tracker):
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        verify_node(output_node, 'tensor', (batch_size, len(
            dataset.language_names)), "LanguageIDModel.run()")

        grad = torch.autograd.grad(
            torch.sum(output_node), inp_xs, allow_unused=True, retain_graph=True)
    ...
    for batch_size, word_length in ((1, 1), (2, 1), (2, 6), (4, 8)):
        ...
        grad = torch.autograd.grad(
            loss_node, inp_xs, allow_unused=True, retain_graph=True)
        for gradient in grad:
            assert gradient != None, "Output returned from LanguageIDModel.run() does not depend on all of the provided inputs (xs)"
...
def check_convolution(tracker):
    ...
    for batch_size in (1, 2, 4):
        inp_x = torch.tensor(
            dataset[:batch_size]['x'], dtype=torch.float, requires_grad=True)
        inp_y = torch.tensor(
            dataset[:batch_size]['label'], dtype=torch.float, requires_grad=True)
        loss = model.get_loss(inp_x, inp_y)
    ...
    for matrix_size in (2, 4, 6):  # Test 3 random convolutions to test convolve() function
        ...
        student_output = models.Convolve(
        ...
        ), "The convolution returned by Convolve() does not match expected output"

    # Partial credit for testing whether convolution function works
...
def check_attention(tracker):
    ...
    for block_size in [2, 4, 16]:
        ...
        k_weight = torch.rand(layer_size, layer_size).reshape(
            layer_size, layer_size)
        q_weight = torch.rand(layer_size, layer_size).reshape(
            layer_size, layer_size)
        v_weight = torch.rand(layer_size, layer_size).reshape(
            layer_size, layer_size)

        # Manually assign attention weights, makes autograding easier
        with torch.no_grad():
            att_block.k_layer.weight = torch.nn.Parameter(
                torch.ones((layer_size, layer_size)) * k_weight)
            att_block.q_layer.weight = torch.nn.Parameter(torch.ones(
                (layer_size, layer_size)).reshape(layer_size, layer_size) * q_weight)
            att_block.v_layer.weight = torch.nn.Parameter(torch.ones(
                (layer_size, layer_size)).reshape(layer_size, layer_size) * v_weight)

        T = input.shape[1]

        expected_output = torch.matmul(att_block.k_layer(input), torch.movedim(
            att_block.q_layer(input), 1, 2))/layer_size**0.5

        expected_output = expected_output.masked_fill(
            att_block.mask[:, :, :T, :T] == 0, float('-inf'))[0]

        output = torch.matmul(nn.functional.softmax(
            expected_output, dim=-1), att_block.v_layer(input))

        assert torch.all(torch.isclose(output, att_block(
            input))), "The output returned by Attention() does not match expected output"
    tracker.add_points(1)
